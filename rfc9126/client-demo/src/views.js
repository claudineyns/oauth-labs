...
function homePage({ clientId }) {
  return layout('OAuth 2.0 — RFC 9126 lab', `
    <h1>Client Demo</h1>
    <p class="subtitle">client_id: <span class="token">${clientId}</span></p>
    <section>
      <h2>Authorization Code Grant + PAR (RFC 9126)</h2>
      <a class="btn primary" href="/login">Login com Authorization Server</a>
      <p class="subtitle" style="margin-top:.5rem">O login empurra os parâmetros da authorization request via POST autenticado (/par) antes de tocar o browser — acompanhe no log de eventos: a chamada ao /par (com todos os parâmetros) e, em seguida, o redirect ao /authorize com só client_id + request_uri.</p>
    </section>
    <section>
