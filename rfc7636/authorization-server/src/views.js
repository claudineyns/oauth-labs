const baseStyle = `
  ...
  * { box-sizing: border-box; }
  ...
  .pkce-box { margin-top:1rem; background:#0f1115; border:1px solid var(--border); border-radius:8px; padding:.6rem .7rem; font-size:.72rem; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; color:var(--muted); word-break:break-all; }
...
function loginPage({ clientName, scope, clientId, redirectUri, state, codeChallenge, codeChallengeMethod, error }) {
  const scopes = (scope || '').split(' ').filter(Boolean);
  const body = `
    <h1>${clientName}</h1>
    <p class="subtitle">solicita acesso à sua conta &mdash; RFC 6749, Authorization Code Grant + PKCE (RFC 7636)</p>
    ...
    <form method="POST" action="/authorize">
      ...
      <div class="pkce-box">code_challenge (${codeChallengeMethod}): ${codeChallenge}</div>
      <div class="actions">
