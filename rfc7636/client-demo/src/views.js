...
function homePage({ clientId }) {
  return layout('OAuth 2.0 — RFC 7636 lab', `
    <h1>Client Demo</h1>
    <p class="subtitle">client_id: <span class="token">${clientId}</span></p>
    <section>
      <h2>Authorization Code Grant + PKCE (RFC 7636)</h2>
    ...
    <section>
      ...
      <a class="btn danger" href="/pkce-demo/start">Interceptar code sem o code_verifier</a>
      <p class="subtitle" style="margin-top:.5rem">Faz um login real, mas troca o code por token usando um <em>code_verifier</em> errado — como faria quem só capturou o <code>code</code> do redirect, sem ter gerado o verifier original.</p>
    </section>
  `);
}
...
function dashboardPage(sess) {
  ...
  return layout('Dashboard', `
    ...
    <p class="subtitle">logado via Authorization Code Grant + PKCE &middot; <a class="link" href="/logout">sair</a></p>
    <section>
      <h2>Tokens</h2>
    ...
    <section>
      <h2>Revogação (RFC 7009)</h2>
      <form method="POST" action="/action/revoke"><button class="danger">Revogar refresh_token (cascata)</button></form>
      <p class="subtitle" style="margin-top:.5rem">Revoga o refresh_token no AS — que também invalida, em cascata, o access_token da mesma família. Depois, tente qualquer botão "Chamar Resource Server" acima para ver o token morto.</p>
    </section>
    <section>
...
function pkceAttackResultPage({ verifierUsed, verifierReal, tokenRes }) {
  return layout('Ataque PKCE', `
    <h1>Interceptação simulada</h1>
    <p class="subtitle">login real, mas a troca do code usou um code_verifier diferente do gerado originalmente — RFC 7636 §4.6</p>
    <section>
      <h2>Verifiers</h2>
      <div class="token">gerado originalmente (nunca saiu do client): ${verifierReal}</div>
      <div class="token" style="margin-top:.4rem">usado nesta troca (simulando o atacante): ${verifierUsed}</div>
    </section>
    <section>
      <h2>Resposta do /token</h2>
      ${fmtResult({ via: 'authorization_code (verifier incorreto)', status: tokenRes.status, body: tokenRes.body })}
    </section>
    <p class="subtitle">${tokenRes.status >= 400 ? 'Como esperado: o AS rejeitou a troca — PKCE cumpriu seu papel.' : 'Inesperado: a troca deveria ter sido rejeitada.'}</p>
    <p><a class="link" href="/">&larr; voltar</a></p>
  `, true);
}
...
module.exports = { homePage, dashboardPage, clientCredentialsResultPage, pkceAttackResultPage, errorPage };
