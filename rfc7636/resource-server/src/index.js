...
const REALM = 'oauth-rfc7636-lab';

const app = express();
app.use(express.urlencoded({ extended: false }));
// observabilidade: todo trafego recebido aqui e machine-to-machine (backend do client-demo)
...
// Extrai e valida o access_token conforme RFC 6750 §2 — header Authorization
// (recomendado), body form-encoded (§2.2, apenas POST) ou query string
// (§2.3, desaconselhado). No maximo UM metodo por requisicao. Sem mudancas
// em relacao ao lab da RFC 6749 — PKCE e revogacao afetam apenas AS.
function requireBearer(requiredScope) {
  return async (req, res, next) => {
    ...
    if (!Object.keys(data).length) {
      res.set('WWW-Authenticate', wwwAuth(', error="invalid_token", error_description="token inexistente, expirado ou revogado"'));
      return res.status(401).json({ error: 'invalid_token', error_description: 'token inexistente, expirado ou revogado' });
    }
