...
function homePage({ clientId, clientName, issuedAt, jkt }) {
  const issuedStr = new Date(issuedAt * 1000).toLocaleString('pt-BR');
  return layout('OAuth 2.0 — RFC 9449 lab', `
    <h1>Client Demo</h1>
    <p class="subtitle">
      client_id: <span class="token">${clientId}</span><br>
      client_name: ${clientName}<br>
      registrado (RFC 7591) em ${issuedStr} — client público, sem client_secret<br>
      tokens vinculados via <strong>DPoP</strong> (RFC 9449) — thumbprint da chave <span class="token">${jkt}</span>
    </p>
    <section>
...
function dashboardPage(sess) {
  ...
  return layout('Dashboard', `
    ...
    <section>
      ...
      <p class="subtitle">token_type: DPoP &middot; escopo: ${sess.scope} &middot; expira ~${expiresAt}</p>
    </section>
    <section>
      <h2>Chamar Resource Server (RFC 9449)</h2>
      <div class="grid">
        <form method="POST" action="/action/call">
          <button>Com prova DPoP (correto)</button>
        </form>
        <form method="POST" action="/action/invalid-bearer">
          <button>Como Bearer simples (sem prova)</button>
        </form>
        <form method="POST" action="/action/replay">
          <button>Reenviar a mesma prova (replay)</button>
        </form>
      </div>
      <p class="subtitle" style="margin-top:.5rem">Acompanhe a montagem/validação nos logs: <code>./containers/logs.sh client</code> e <code>./containers/logs.sh rs</code>.</p>
    </section>
    <section>
      <h2>Renovação</h2>
...
function clientCredentialsResultPage({ tokenRes, rsResult }) {
  return layout('Client Credentials', `
    ...
    <p class="subtitle">chamada machine-to-machine, token vinculado via DPoP &mdash; RFC 6749 §4.4 + RFC 9449</p>
...
module.exports = { homePage, dashboardPage, clientCredentialsResultPage, errorPage };
